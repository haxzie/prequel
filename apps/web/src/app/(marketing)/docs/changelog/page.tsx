...
/**
 * The releases, as a vertical timeline, inside the docs.
 *
 ...
 *
 * It lived at `/changelog` with a measure of its own until the docs arrived;
 * the shell now supplies the measure, and `next.config.ts` redirects the old
 * address here.
 */
export default async function Changelog() {
  const [shipped, sections] = await Promise.all([published(), outline()]);

  return (
    <DocsShell sections={sections} current="changelog">
      <header>
        ...
        <p className="mt-4 text-lg leading-relaxed text-pretty text-muted">
          Prequel updates itself, so you are usually on the newest of these already. The app tells
          you when there is one waiting.
        </p>
      </header>
